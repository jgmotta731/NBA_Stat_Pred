...
bref_injuries <- function() {
  ...
  tbl <- rvest::html_element(pg, "table") |> rvest::html_table()
  dplyr::as_tibble(tbl) |>
    dplyr::rename(
      player = Player,
      team = Team,
    ...
    dplyr::mutate(
      description = stringr::str_squish(as.character(description))
    ) |>
    dplyr::filter(stringr::str_starts(description, "Out")) |>
...
player_raw <- load_nba_player_box(
  seasons = 2022:most_recent_nba_season()) %>%
