...
# Includes multiple diagnostics for Windows Task Scheduler
...
# Lock file to avoid overlapping runs (treat less than 30 minutes as active)
if (file.exists(lockfile)) {
  age_hours <- as.numeric(difftime(Sys.time(), file.info(lockfile)$mtime, units = "hours"))
  if (!is.na(age_hours) && age_hours < 0.5) {
...
message("update_nba_data.R completed.")
quit(status = 0)